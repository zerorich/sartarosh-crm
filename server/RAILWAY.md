...
|----------|-------------------|-------|
| `NODE_ENV` | `production` | **Обязательно** для prod. Без Gmail в production OTP-письма не отправятся. |
...
| `GMAIL_USER` | `your@gmail.com` | Gmail-аккаунт для SMTP. |
| `GMAIL_APP_PASSWORD` | *(App Password)* | [App Password](https://myaccount.google.com/apppasswords) — не обычный пароль Gmail. |
| `SMTP_FROM` | `Sartarosh <your@gmail.com>` | Отправитель писем. Если пусто — используется `GMAIL_USER`. |
...
Без `GMAIL_USER` + `GMAIL_APP_PASSWORD` в `NODE_ENV=production` отправка OTP **упадёт с ошибкой**.

1. Включите 2FA в Google Account.
2. Создайте [App Password](https://myaccount.google.com/apppasswords).
3. Задайте переменные в Railway Variables API-сервиса.
